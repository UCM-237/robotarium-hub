...
log_filename = 'robot_telemetry.log'
...
# Handler para archivo (Rota al llegar a 5MB, mantiene 3 archivos de respaldo)
file_handler = RotatingFileHandler(log_filename, maxBytes=5*1024*1024, backupCount=3)
file_handler.setFormatter(formatter)
file_handler.setLevel(logging.DEBUG) # En el archivo guardamos TODO

# Handler para consola (Para ver lo importante mientras ejecutas)
console_handler = logging.StreamHandler()
console_handler.setFormatter(formatter)
console_handler.setLevel(logging.INFO) # En consola solo INFO y errores
...
logging.info("SISTEMA DE LOGGING INICIALIZADO - Archivo: " + log_filename)
# Configure logs
logging.basicConfig(
    level=logging.DEBUG,
...
class Robot:
  ...
  def save_to_csv(self, data_dict):
    file_exists = os.path.isfile('telemetry_data.csv')
    with open('telemetry_data.csv', 'a', newline='') as f:
    ...
    while(True):
      ...
      if len(self.Position)>0:
        ...
        TODO: Se puede mejorar teniendo en cuenta el rumbo actual del robot y el nuevo rumbo recomendado para decidir si gira a la derecha o a la izquierda, buscando siempre la dirección de giro más corta.
         --------------------------------------------------------------------------------------------------'''
        v_actual = 0.1 # Velocidad lineal estimada (m/s)
        ...
        if dist <= 0.5 and dist >= 0.3:
          ...
          logging.warning(f"ZONA DE COLISIÓN: Dist Futura: {dist:.3f}m | Heading Evasión: {target_yaw:.2f} | Yaw Actual: {heading:.2f}")
          ...
          logging.debug(f"TRAYECTORIA SEGURA: Margen: {dist:.3f}m")
  ...
  def move_robot(self, v_left, v_right) -> None:
    ...
    #logging.info(f"CMD ARDUINO: v_left={v_left}, v_right={v_right}")
  ...
  def move_wheels(self,pwm_left, pwm_right) -> None:
    ...
    logging.info(f"CMD ARDUINO:  PWM_L={pwm_left}, PWM_R={pwm_right}")
    
  def stop_robot(selfop) -> None:
    '''Stop the robot'''
    len=0
  ...
  def turn_robot(self, angle) -> None:
    ...
    logging.info(f"CMD ARDUINO: TURN {angle}")
  ...
  def on_data(self, topic: str, message: str) -> None:
    ...
    try:
      ...
      #logging.debug(f"Message received: {message} in topic {topic}")
      if topic=="control"  and self.IgnoreControlCommunication == False:
        ...
        self.Pos=True
        #logging.debug(f"POSICION RECIBIDA: x={self.Position[agent]['x']}, y={self.Position[agent]['y']}, yaw={self.Position[agent]['yaw']}")
        telemetry = {"timestamp": time.time(), "x": self.Position[agent]['x'], "y": self.Position[agent]['y'] , "yaw": self.Position[agent]['yaw']}
        self.save_to_csv(telemetry)
