        ...
        if topic == "vision/stitched":
            try:
                if not self.image_queue.full():
                    self.image_queue.put_nowait(message)
                else:
                ...
                if self.missed_frames % 10 == 0:
                    logger.warning(f"[ADVERTENCIA] Se han perdido {self.missed_frames} frames por saturación de la cola.")
        ...
        while True:
            try:
                ...
                try:
                    ...
                    continue
                if img_b64:
                    if "vision" in img_b64 and "{" not in img_b64:
                        logger.warning(f"[ADVERTENCIA] Posible error en payload de imagen: {img_b64[:30]}...")
                        self.image_queue.task_done()
                        continue
