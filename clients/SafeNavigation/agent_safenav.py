...
class BouncerRobot:
    def __init__(self, agent: Agent) -> None:
        ...
        #TODO Nacho: crear una variable para guardar la distancia del IR
        self.distancia_infrarojo = 0.0
    ...
    def on_data(self, topic: str, message: str) -> None:
        #logger.critical(f"Incoming data. Topic {topic}, mensaje {message}")
        ...
        #TODO Nacho: Añadir la recepción de la distancia del IR      
        elif topic == f"agent/{self.robot_id}/distance": 
            try:
                raw_data = json.loads(message)
                logger.critical(raw_data)

                # Suponiendo que el JSON tiene una clave llamada "distance"
                self.distancia_infrarojo = float (raw_data["Distancia"])

                logger.debug(f"Distancia recibida: {self.distancia_infrarojo }")
                ...
                #  MQTT_agent.send('agent/8/move', { 'v': v_actual , 'w': w_actual })
    ...
    def actualizar_fsm(self,x,y,theta):
        ...
        logger.warning(f"Distancia infrarrojo: {self.distancia_infrarojo}")
        # 2. Evaluación de la Máquina de Estados
        #TODO Nacho: añadir las condiciones de la distancia del IR
        if self.fsm == RobotState.AVANZA:
            # FILTRO 1: Seguridad Absoluta. Si por colisión o inercia está pegado a un muro, va atrás.
            if (np.abs(dist_absoluta) <= self.danger_distance) or ( self.distancia_infrarojo <= 10 and  self.distancia_infrarojo !=-1 ): 
            ...
            # FILTRO 2: Comportamiento Billar. Si la trayectoria colisionará pronto, calcula reflexión.
                ...
                self.giro_terminado=False
            elif ( self.distancia_infrarojo <= 20 and  self.distancia_infrarojo !=-1 ):
                self.target_theta = 0.2
                self.fsm =RobotState.GIRA
                self.giro_terminado = False
...
if __name__ == "__main__":
   # Configuración del Agente
    ...
    bouncer_agent.setup_subscriptions(topic)
    logger.info(f"Suscrito a topic {topic}")
    #TODO Nacho: añadir la suscripcion al topic distnacia IR
    #client.subscribe("agent/8/distance")   
    topic=b'agent/6/distance'
    bouncer_agent.setup_subscriptions(topic)
    logger.warning(f"Suscrito a topic {topic}")
    time.sleep(3)
