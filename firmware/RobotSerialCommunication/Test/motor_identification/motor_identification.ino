...
// -----------------------------------------------------------------------------
  ...
  const int pin_left_motor_direction  = 6;
  const int pin_left_motor_enable     = 7;
...
double setpointW[2] = {0, 0}; // setpoint in { 0 U (1.8, 3.6) } 
...
void loop() {
  ...
  if(dt_us >= sampling_time_us) {
    ...
    last_time_us = current_time_us;                                   1;
  }
}
