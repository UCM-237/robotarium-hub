...
//UDP variables setup
/*-----------ip setup-----------------*/
IPAddress ip_arduino1(192,168,78,7);
IPAddress ip_server(192,168,78,2);
WiFiUDP Udp;

unsigned int localPort = 4245;      // local port to listen on
int ServerPort = 4240;
...
void feedForwardD()
{
    ...
    {
      PWM_D=round((setpointWD - 1.257)/0.057);
      if(PWM_D>MAXPWM){
       PWM_D=MAXPWM;
      }
       else if(PWM_D<MINPWM){
...
{   if(setpointWI==0.)
    {
      ...
      PWM_I=round((setpointWI - 0.332)/0.060);
