...
//UDP variables setup
/*-----------ip setup-----------------*/
IPAddress ip_arduino1(192,168,78,5);
...
void op_vel_robot(bool serialCom){
  ...
  {
    //shortToBytes(b,&operation_send.data[16]);
    operation_send.data[16]='b';
  }
  if(backI){
  ...
  {
    //shortToBytes(b,&operation_send.data[18]);
    operation_send.data[17]='b';
  }*/
 ...
 // shortToBytes(b,&operation_send.data[20]);
  ...
  if(serialCom){
    Serial.write((char*)operation_send.data,18);
...
int pidD(double wD)
{
  ...
  if(aux>=0.30){
    ...
    if(cumErrorD>MAXCUMERROR || cumErrorD <-MAXCUMERROR)
      if(cumErrorD >0){
...
void feedForwardD()
{
    ...
    {
      PWM_D=round((setpointWD - 0.6866)/0.0648);
      if(PWM_D>MAXPWM){
       PWM_D=MAXPWM;
      }
       else if(PWM_D<MINPWM){
...
{   if(setpointWI==0.)
    {
      ...
      PWM_I=round((setpointWI -1.1682 )/0.0606);
